<?
	include_once("../lib/mconfig.php");

	$Id = $_SESSION['Account_Id'];
	$oMysql = new cDbConn();
	$oMysql->DbConn();
	
	$arSQL = "SELECT Id, Title, Content, Images, Type, HashTag, Talent_Id ";
	$strTB = " FROM Blogs";
	$strWHERE = " WHERE 1=1 AND Id ={$_REQUEST['id']}";
	$strOrder = " LIMIT 1";
		
	$arSQL.= $strTB.$strWHERE.$strOrder;

	$oMysql->excute("SET NAMES UTF8");

	$rs		= $oMysql->excute($arSQL);
	
	$mDATA = $oMysql->fetch("array");

	$arrIU = explode('|', $mDATA['Images']);
	$num = sizeof($arrIU);
...
<div class="body">
	...
	<input type="hidden" name="Blog_Id" value="<?=$_REQUEST['id']?>">
	<input type="hidden" name="Talent_Id" value="<?=$mDATA['Talent_Id']?>">
	<input type="hidden" name="atype" value="<?=$mDATA['Type']?>">
	제목 : <input type="text" style="width:500px;" name="title" value="<?=$mDATA['Title']?>">
	<br>
	타입 : <select style="width:200px;" name="type">
				<option value="1" <?if($mDATA['Type']==1)echo "selected";?>>수업후기</option>
				<option value="2" <?if($mDATA['Type']==2)echo "selected";?>>수업TIP</option>
				<option value="0" <?if($mDATA['Type']==0)echo "selected";?>>포트폴리오</option>
				<?if($Id=='22'){?>
				<option value="4" <?if($mDATA['Type']==4)echo "selected";?>>알림</option>
				<option value="5" <?if($mDATA['Type']==5)echo "selected";?>>이벤트</option>
				<option value="6" <?if($mDATA['Type']==6)echo "selected";?>>튜터소식</option>
				<?}?>
			</select>
	<br>
	<br>
